...
- **State**: lifecycle states, transitions, guards, events. Prefer explicit `state` declarations. In strict AEther dark diagrams, use styled Start/End states instead of unstyleable `[*]` pseudo-nodes.
...
- **C4**: architecture context/container/component diagrams. Requires C4 macros and therefore an include policy decision; use `!include <C4/C4_Container.puml>` for container views rather than redefining C4 macros inline.
